...
class PublicDataPoolTest(TransactionTestCase):
    ...
    def test_create_unsub(self):
        ...
        self.assertRedirects(
            ...
            reverse('rawdata.restricted') + '?' + urlencode(
                {'next': '/rawdata/publicdatapools/share/%d/' % rawimage_id}),
            status_code=302, target_status_code=200)
        self.client.logout()
    ...
    def test_add_data_unsub(self):
        ...
        self.assertRedirects(
            response,
            reverse('rawdata.restricted') + '?' + urlencode(
                {'next': '/rawdata/publicdatapools/%d/add-data/' % pool.id}),
            status_code=302, target_status_code=200)
        self.client.logout()
    ...
    def test_remove_data_anon(self):
        ...
        self.assertRedirects(
            response,
            'http://testserver/accounts/login/?next=/rawdata/publicdatapools/%d/remove-data/%d/' % (
                pool.id, rawimage_id),
            status_code=302, target_status_code=200)
    ...
    def test_remove_data_unsub(self):
        ...
        self.assertRedirects(
            response,
            reverse('rawdata.restricted') + '?' + urlencode(
                {'next': '/rawdata/publicdatapools/%d/remove-data/%d/' % (pool.id, rawimage_id)}),
            status_code=302, target_status_code=200)
        self.client.logout()
