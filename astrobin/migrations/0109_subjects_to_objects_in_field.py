...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for i in orm['astrobin.image'].objects.all():
            if i.subjects.count():
                subjects = i.subjects.all()
                objects = ', '.join(['%s%s' % (fix_catalog(x.catalog) if x.catalog else '', fix_name(x.name.strip())) for x in i.subjects.all()])

                i.objects_in_field = objects[:512]
                i.subjects.clear()
                i.save()
    ...
    models = {
        'actstream.action': {
            ...
            'timestamp': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 10, 25, 10, 32, 46, 980935)'}),
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 10, 25, 10, 32, 46, 977813)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 10, 25, 10, 32, 46, 977739)'}),
