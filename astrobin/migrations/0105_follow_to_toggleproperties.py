...
class Migration(DataMigration):
    ...
    def backwards(self, orm):
        ...
        for u in orm['auth.user'].objects.all():
            tps = orm['toggleproperties.toggleproperty'].objects.filter(property_type = 'follow', user = u)
            for tp in tps:
                if tp.content_type == user_ct:
                    u2 = orm['auth.user'].objects.get(pk = tp.object_id)
                    u.userprofile.follows.add(u2.userprofile)

                if tp.content_type == gear_ct:
                    try:
                        g = orm['astrobin.gear'].objects.get(pk = tp.object_id)
                        u.userprofile.follows_gear.add(g)
                    except orm['astrobin.gear'].DoesNotExist:
                        pass

                if tp.content_type == subj_ct:
                    s = orm['astrobin.subject'].objects.get(pk = tp.object_id)
                    u.userprofile.follows_sujects.add(s)


        tp = orm['toggleproperties.toggleproperty'].objects.filter(property_type = 'follow').delete()
    ...
    models = {
        'actstream.action': {
            ...
            'timestamp': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 10, 20, 16, 45, 2, 17295)'}),
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 10, 20, 16, 45, 1, 972424)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 10, 20, 16, 45, 1, 972181)'}),
