...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing M2M table for field retailers on 'Gear'
        db.delete_table('astrobin_gear_retailers')

        # Adding M2M table for field retailed on 'Gear'
        db.create_table('astrobin_gear_retailed', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('gear', models.ForeignKey(orm['astrobin.gear'], null=False)),
            ('retailedgear', models.ForeignKey(orm['astrobin.retailedgear'], null=False))
        ))
        db.create_unique('astrobin_gear_retailed', ['gear_id', 'retailedgear_id'])


    def backwards(self, orm):
        
        # Adding M2M table for field retailers on 'Gear'
        db.create_table('astrobin_gear_retailers', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('gear', models.ForeignKey(orm['astrobin.gear'], null=False)),
            ('retailedgear', models.ForeignKey(orm['astrobin.retailedgear'], null=False))
        ))
        db.create_unique('astrobin_gear_retailers', ['gear_id', 'retailedgear_id'])

        # Removing M2M table for field retailed on 'Gear'
        db.delete_table('astrobin_gear_retailed')
