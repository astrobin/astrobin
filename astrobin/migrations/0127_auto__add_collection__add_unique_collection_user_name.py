...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('astrobin_collection', (
        ...
        db.send_create_signal('astrobin', ['Collection'])

        # Adding M2M table for field images on 'Collection'
        db.create_table('astrobin_collection_images', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('collection', models.ForeignKey(orm['astrobin.collection'], null=False)),
            ('image', models.ForeignKey(orm['astrobin.image'], null=False))
        ))
        db.create_unique('astrobin_collection_images', ['collection_id', 'image_id'])

        # Adding unique constraint on 'Collection', fields ['user', 'name']
        db.create_unique('astrobin_collection', ['user_id', 'name'])


    def backwards(self, orm):
        # Removing unique constraint on 'Collection', fields ['user', 'name']
        db.delete_unique('astrobin_collection', ['user_id', 'name'])

        # Deleting model 'Collection'
        db.delete_table('astrobin_collection')

        # Removing M2M table for field images on 'Collection'
        db.delete_table('astrobin_collection_images')
