...
class ImageRetailerAffiliatesWhenOwnerIsPremiumAutorenewAndDoesNotAllowTest(TestCase):
    def setUp(self) -> None:
        owner = Generators.user()
        UserProfile.objects.filter(user=owner).update(allow_retailer_integration=False)
        owner.refresh_from_db()
        self.owner_us = Generators.premium_subscription(owner, SubscriptionName.PREMIUM_CLASSIC_AUTORENEW)
