...
class BroadcastEmailAdminRecoveredImagesNoticeDeTest(TestCase):
    @patch("astrobin.tasks.send_broadcast_email.delay")
    def test_submit_recovered_images_notice_de_no_users(self, taskMock):
        request = RequestFactory().get("/")

        email = BroadcastEmail.objects.create(subject="test")
        admin = BroadcastEmailAdmin(model=BroadcastEmail, admin_site=AdminSite())

        admin.submit_recovered_images_notice_de(request, BroadcastEmail.objects.filter(pk=email.pk))

        taskMock.assert_not_called()

    @patch("astrobin.tasks.send_broadcast_email.delay")
    def test_submit_recovered_images_notice_de_no_recovered_images(self, taskMock):
        request = RequestFactory().get("/")
    ...
    @patch("astrobin.tasks.send_broadcast_email.delay")
    def test_submit_recovered_images_notice_de_wrong_language(self, taskMock):
        request = RequestFactory().get("/")
    ...
    @patch("astrobin.tasks.send_broadcast_email.delay")
    def test_submit_recovered_images_notice_de_already_sent(self, taskMock):
        request = RequestFactory().get("/")
    ...
    def test_submit_recovered_images_notice_de_sent(self, messageUserMock, taskMock):
        request = RequestFactory().get("/")

        email = BroadcastEmail.objects.create(subject="test")
        admin = BroadcastEmailAdmin(model=BroadcastEmail, admin_site=AdminSite())
        user = Generators.user()
        user.userprofile.language = 'de'
        user.userprofile.save(keep_deleted=True)
        Generators.image(user=user, recovered=datetime.now())
