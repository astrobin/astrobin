...
def image_detail(request, id, r):
    ...
    if deep_sky_acquisitions:
        ...
        deep_sky_data = (
            ...
            (_('Darks'), '~%d' % (int(reduce(lambda x, y: x + y, dsa_data['darks'])) / len(dsa_data['darks'])) if dsa_data['darks'] else 0),
            (_('Flats'), '~%d' % (int(reduce(lambda x, y: x + y, dsa_data['flats'])) / len(dsa_data['flats'])) if dsa_data['flats'] else 0),
            (_('Flat darks'), '~%d' % (int(reduce(lambda x, y: x + y, dsa_data['flat_darks'])) / len(dsa_data['flat_darks'])) if dsa_data['flat_darks'] else 0),
            (_('Bias'), '~%d' % (int(reduce(lambda x, y: x + y, dsa_data['bias'])) / len(dsa_data['bias'])) if dsa_data['bias'] else 0),
