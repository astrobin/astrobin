...
class CameraViewSet(EquipmentItemViewSet):
    ...
    def get_queryset(self) -> QuerySet:
        ...
        if camera_max_cooling_filter:
            ...
            if max_cooling_object.get('from') is not None or max_cooling_object.get('to') is not None:
                queryset = queryset.filter(
                    max_cooling__isnull=False,
                    max_cooling__gte=max_cooling_object.get('from')
                    if max_cooling_object.get('from') is not None
                    else 0,
                    max_cooling__lte=max_cooling_object.get('to')
                    if max_cooling_object.get('to') is not None
                    else sys.maxsize
                )
        ...
        if camera_back_focus_filter:
            ...
            if back_focus_object.get('from') is not None or back_focus_object.get('to') is not None:
                queryset = queryset.filter(
                    back_focus__isnull=False,
                    back_focus__gte=back_focus_object.get('from')
                    if back_focus_object.get('from') is not None
                    else 0,
                    back_focus__lte=back_focus_object.get('to')
                    if back_focus_object.get('to') is not None
                    else sys.maxsize
                )
        ...
        if camera_sensor_quantum_efficiency_filter:
            ...
            if sensor_quantum_efficiency_object.get('from') is not None \
                ...
                queryset = queryset.filter(
                    ...
                    if sensor_quantum_efficiency_object.get('to') is not None
                    else sys.maxsize
                )
        ...
        if camera_sensor_pixel_size_filter:
            ...
            if sensor_pixel_size_object.get('from') is not None or sensor_pixel_size_object.get('to') is not None:
                queryset = queryset.filter(
                    ...
                    if sensor_pixel_size_object.get('to')
                    else sys.maxsize
                )
        ...
        if camera_sensor_pixel_width_filter:
            ...
            if sensor_pixel_width_object.get('from') is not None or sensor_pixel_width_object.get('to') is not None:
                queryset = queryset.filter(
                    ...
                    if sensor_pixel_width_object.get('from') is not None
                    else 0,
        ...
        if camera_sensor_pixel_height_filter:
            ...
            if sensor_pixel_height_object.get('from') is not None or sensor_pixel_height_object.get('to') is not None:
                queryset = queryset.filter(
                    ...
                    if sensor_pixel_height_object.get('to') is not None
                    else sys.maxsize
                )
        ...
        if camera_sensor_read_noise_filter:
            ...
            if read_noise_object.get('from') is not None or read_noise_object.get('to') is not None:
                queryset = queryset.filter(
        ...
        if camera_sensor_adc_filter:
            ...
            if adc_object.get('from') is not None or adc_object.get('to') is not None:
                queryset = queryset.filter(
                    ...
                    sensor__adc__gte=adc_object.get('from') if adc_object.get('from') is not None else 0,
                    sensor__adc__lte=adc_object.get('to') if adc_object.get('to') is not None else sys.maxsize
                )
