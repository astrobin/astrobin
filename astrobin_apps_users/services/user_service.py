...
class UserService:
    ...
    def get_all_images(self) -> QuerySet:
        ...
        return Image.objects_including_wip.filter(user=self.user).union(
            Image.objects_including_wip.filter(collaborators=self.user)
        )

    def get_public_images(self) -> QuerySet:
        from astrobin.models import Image
        return Image.objects.filter(user=self.user).union(Image.objects.filter(collaborators=self.user))

    def get_wip_images(self) -> QuerySet:
        from astrobin.models import Image
        return Image.wip.filter(user=self.user).union(Image.wip.filter(collaborators=self.user))
